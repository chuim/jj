...
fn test_fix_one_file() {
    ...
    let new_commit_a = repo
        ...
        .unwrap();
    assert_eq!(new_commit_a.tree(), expected_tree_a);
}
...
fn test_unchanged_file_is_fixed() {
    ...
    let new_commit_b = repo
        ...
        .unwrap();
    assert_eq!(new_commit_b.tree(), expected_tree_b);
}
...
fn test_already_fixed_descendant() {
    ...
    let new_commit_a = repo
        ...
        .get_commit(summary.rewrites.get(&commit_a).unwrap())
        .unwrap();
    assert_eq!(new_commit_a.tree(), tree2);
    let new_commit_b = repo
        .store()
        .get_commit(summary.rewrites.get(&commit_a).unwrap())
        .unwrap();
    assert_eq!(new_commit_b.tree(), tree2);
}
