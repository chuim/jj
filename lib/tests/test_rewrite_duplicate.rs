...
fn test_duplicate_linear_contents() {
    ...
    assert_eq!(stats.duplicated_commits[commit_c.id()].tree(), tree_2);
    assert_eq!(stats.duplicated_commits[commit_e.id()].tree(), tree_2);

    // Duplicate onto multiple parents
    let mut tx = repo.start_transaction();
    let stats = duplicate_onto(&mut tx, &[commit_d.id()], &[commit_c.id(), commit_b.id()]);
    assert_eq!(stats.duplicated_commits[commit_d.id()].tree(), tree_2);
