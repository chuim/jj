...
fn test_root() {
    ...
    assert_eq!(new_tree, wc_commit.tree());
    assert_eq!(new_tree, repo.store().empty_merged_tree());
}
...
fn test_reset() {
    ...
    assert!(!wc.file_states().unwrap().contains_path(ignored_path));
    let new_tree = test_workspace.snapshot().unwrap();
    assert_eq!(new_tree, tree_without_file);
    ...
    assert!(wc.file_states().unwrap().contains_path(ignored_path));
    let new_tree = test_workspace.snapshot().unwrap();
    assert_eq!(new_tree, tree_with_file);
}
...
fn test_snapshot_racy_timestamps() {
    ...
    for i in 0..100 {
        ...
        let (new_tree, _stats) = locked_ws
            ...
            .block_on()
            .unwrap();
        assert_ne!(new_tree, previous_tree);
...
fn test_fsmonitor() {
    ...
    assert_eq!(*tree_state.current_tree(), repo.store().empty_merged_tree());
